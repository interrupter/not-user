const { log } = require("not-log")(module, "Model"),
    ...
    { notError, notValidationError, notRequestError } = require("not-error"),
...
module.exports.thisStatics = {
    async loadSafe(id) {
        const result = await this.find(
            ...
            "_id username email emailConfirmed role active"
        ).exec();
    ...
    toggleActive: async function (id) {
        try {
            ...
            if (e instanceof notRequestError) {
                throw e;
            } else {
                throwUserNotFound(
                    { id: [phrase("user_not_found")] },
                    { id },
                    e
                );
            }
            ...
            return this.findOneAndUpdate(
                ...
                .then(async (item) => {
                    if (typeof item !== "undefined" && item !== null) {
                        ...
                        throw new notError(
                            "-version not saved, empty response",
                            { _id: data._id, safeData }
                        );
                    }
                });
        } else {
    ...
    extractSafeFields(action, data, roles, actorId, system = false) {
        ...
        let fields = this.getSafeFieldsForRoleAction(
            action,
            roles,
            this.isOwner(data, actorId),
            system
        );
        log(action, fields);
        let result = {};
    ...
    isOwner(data, user_id) {
        ...
        return (
            objHas(data, "_id") && data._id.toString() === user_id.toString()
        );
    },
...
module.exports.thisMethods = {
    encryptPassword(password) {
        return crypto
            .createHmac(DEFAULT_HASH_ALGO, this.salt)
            .update(password)
            .digest("hex");
    },
    createNewPassword() {
        let pass =
            module.exports[module.exports.thisModelName].randomPassword();
        this.password = pass;
        return pass;
    },
    ...
    Update(roles, actorId, system = false) {
        return module.exports[module.exports.thisModelName].Update(
            this.toObject(),
            roles,
            actorId,
            system
    ...
    isOwner(user_id) {
        return module.exports[module.exports.thisModelName].isOwner(
            this.toObject(),
            user_id
        );
    },
